...
export class ChartJsComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    let canvas = $('#areaChart').get(0) as HTMLCanvasElement;
    const areaChartCanvas = canvas.getContext('2d');
...
<%if(datasets[i].label){%><%=datasets[i].label%><%}%></li><%}%></ul>`,
    ...
    // - LINE CHART -
    // --------------
    canvas = $('#lineChart').get(0) as HTMLCanvasElement;
    const lineChartCanvas          = canvas.getContext('2d');
    ...
    // -------------
    // Get context with jQuery - using jQuery's .get() method.
    canvas = $('#pieChart').get(0) as HTMLCanvasElement;
...
<span style="background-color:<%=segments[i].fillColor%>"></span><%if(segments[i].label){%><%=segments[i].label%><%}%></li><%}%></ul>`
    ...
    // - BAR CHART -
    // -------------
    canvas = $('#barChart').get(0) as HTMLCanvasElement;
